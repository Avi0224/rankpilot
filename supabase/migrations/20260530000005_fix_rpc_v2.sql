-- Fix for predict_colleges_v2 404/PGRST202 errors
-- This script creates the predict_colleges_v2 function if it doesn't exist,
-- or wraps get_cutoffs if that is the preferred internal source.

-- First, ensure the function signature matches exactly what the frontend sends
CREATE OR REPLACE FUNCTION public.predict_colleges_v2(
  p_rank integer,
  p_category text,
  p_quota text,
  p_gender text,
...
AS $$
BEGIN
  -- Defensive Logging (simulated via NOTICE or just handled by the query)
  -- In production Supabase, you can check Logs for function execution.
...
GRANT EXECUTE ON FUNCTION public.predict_colleges_v2 TO anon, authenticated;
